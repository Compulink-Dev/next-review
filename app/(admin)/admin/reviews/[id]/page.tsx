...
function Review() {
  ...
  const employeeReviews = previousReviews.filter(
    (prev) =>
      ...
      prev.employeeId._id === review.employeeId._id
  );
  ...
  console.table(
    previousReviews.map((prev) => ({
      prevId: prev.employeeId?._id,
      employeeName: prev.employeeId?.name,
      rating: prev.rating,
    }))
  );

  console.log("Current Review Employee ID:", review?.employeeId?._id);

  console.log("Employee : ", employeeReviews);
  console.log("Previous : ", previousReviews);

  return (
    <div>
      <div className="flex items-center justify-between">
        <Title title={`Review for ${review?.employeeId?.name}`} />
        <BackButton />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
            ...
            {session?.user.role === "companyAdmin" && (
              <div className="mt-4 flex gap-4">
                <Button
                  ...
                  onClick={() =>
                    router.push(`/dashboard/employees/${review.employeeId._id}`)
                  }
                >
                  View Employee
                </Button>
        ...
        <div className="bg-white p-6 shadow rounded-lg h-full">
          <Title
            subtitle={` Previous Reviews for ${review?.employeeId?.name}`}
          />
          {employeeReviews.length > 0 ? (
            <div className="space-y-4 mt-4">
              {employeeReviews.map((prevReview) => (
                <div
                  key={prevReview._id}
